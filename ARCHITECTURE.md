...
**Tab-Bar-Blur:** neue Dependency `expo-blur` (`BlurView` als `tabBarBackground`, `tabBarStyle.backgroundColor: 'transparent'`). **`tabBarStyle.position: 'absolute'` ist Pflicht** (Expo Routers Tabs-Doku: "When using BlurView, make sure to set `position: 'absolute'` in `tabBarStyle` as well") — der erste Implementierungsversuch nutzte bewusst `'relative'`, um die `Screen`-`edges`-Konvention der drei Tab-Screens nicht anfassen zu müssen ("the tab bar is the bottom buffer"), was die App bei jedem Start sofort abstürzen ließ (Expo Routers Error-Boundary zeigte nur "Something went wrong"). Fix: `position: 'absolute'` plus eine neue Konstante `TAB_BAR_CLEARANCE_BASE` (`src/shared/theme/icons.ts`) — jeder Tab-Screen packt seinen Inhalt jetzt in einen zusätzlichen `View` mit `paddingBottom: TAB_BAR_CLEARANCE_BASE + useSafeAreaInsets().bottom`, damit nichts mehr unter der jetzt schwebenden Bar verschwindet. `@react-navigation/bottom-tabs`s eigener `useBottomTabBarHeight()`-Hook ist hier nicht direkt importierbar (nur intern in `expo-router`s Build vendored, kein Teil von dessen öffentlicher API) — die Konstante ist eine bewusste Näherung, nicht pixelgenau am echten Gerät vermessen.

**Manueller Light/Dark-Umschalter** (Profil-Tab, Karte "Erscheinungsbild", `System`/`Hell`/`Dunkel` als wiederverwendete `FilterChip`s): `src/store/theme.store.ts` (Zustand + `persist`, gleiches Muster wie `reminders.store.ts`) hält die Präferenz und ruft bei jeder Änderung sowie beim `onRehydrateStorage`-Callback (also nach jedem Kaltstart) `colorScheme.set(...)` aus dem `nativewind`-Paket auf. Das ist kein eigenes Theming-System — `colorScheme.set()` ruft intern React Natives eigenes `Appearance.setColorScheme()` auf, was **sowohl** NativeWinds `dark:`-Klassen **als auch** jeden bestehenden `useColorScheme()`-Aufruf aus `react-native` selbst aktualisiert (beide lesen aus demselben `Appearance`-Modul) — keiner der ca. 13 Call-Sites, die weiterhin direkt `useColorScheme` aus `react-native` importieren, musste dafür angefasst werden.
