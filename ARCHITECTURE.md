...
- **Nicht auf einem echten Gerät verifiziert** (wie der Rest der App bisher, siehe `PROJECT_STATUS.md`) — nur gegen `tsc`/`lint`/`expo export`-Bundling geprüft. Visuelle Feinheiten (exakte Abstände, Fokus-Ringe, Press-Feedback-Intensität) sind ungetestet.
