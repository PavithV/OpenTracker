...
Stand: 2026-07-24. Phase 1 komplett, Design System komplett, **Phase 2 (Punkte 1–8) vollständig abgeschlossen** plus zwei ungeplante Zwischenschritte (Routinen-Liste, "Routine starten"). Der komplette Kernkreislauf **Registrieren → Routine erstellen → Routine starten → Workout durchführen → Workout beenden → Workout-Historie ansehen → Workout-Detail ansehen → Übungsdetail ansehen → Profil-Aggregate ansehen** ist Ende-zu-Ende gegen die echte Supabase-Datenbank verifiziert. Zwei reale, vorbestehende Bugs wurden dabei gefunden und gefixt (siehe unten). **Ausstehend ist nur noch der echte On-Device-Test** durch den Nutzer selbst. **Phase 3, Punkte 1–4 (PR-Diagramm, One Rep Max, Muskel-Split, Rekorde-Übersicht) sind vollständig umgesetzt** — die aktuelle Phase-3-Planungsgrundlage ist damit komplett abgearbeitet. Punkt 2 brachte eine dritte Live-Migration (`0003_add_estimated_1rm_personal_record.sql`), Punkt 4 einen neuen Top-Level-Screen (`app/records.tsx`). Vor dem Einstieg in Phase 4 wurde außerdem eine echte, bisher undokumentierte Lücke nachgezogen: runde Übungsbilder fehlten im Home-Bereich (und in Workout-Detail/Übungshistorie) — siehe `TODO.md`, Abschnitt „Nachgezogene Lücke". **Für Phase 4 existiert eine technische Bestandsaufnahme** (sieben Punkte laut `ROADMAP.md`, siehe `TODO.md`); der Nutzer hat sich für Punkt 1 (Notizen) als Einstieg entschieden. **Phase 4, Punkt 1 (Notizen) ist umgesetzt** — alle drei bereits im Schema vorhandenen `notes`-Spalten (Routine, Workout, Workout-Übung) sind jetzt an UI angebunden.
...
- Phase-3-Punkt 4 (Rekorde aktiv nutzen) umgesetzt: neues Feature `src/features/records/` mit `getPersonalRecords()` (eine Query über `personal_records` für beide `record_type`-Werte + eine Query über `exercises` für die Namen, in JS pro Übung zusammengeführt, alphabetisch sortiert) und neuem Top-Level-Screen `app/records.tsx` (in `app/_layout.tsx`s `Stack.Protected`-Gruppe registriert), erreichbar über einen neuen „Rekorde ansehen"-Button im Profil-Tab. Tap auf eine Zeile führt zurück zu `exercise/[id]` — schließt den Kreis zur Einzelansicht aus Punkt 7. **Damit ist die komplette Phase-3-Planungsgrundlage (Punkte 1–4) abgeschlossen.** Gegen die Live-DB verifiziert: zwei Übungen mit je einem Satz in einem Test-Workout bestätigten sowohl die korrekte Zusammenführung von `max_weight`+`estimated_1rm` pro Übung als auch die alphabetische Sortierung, Test-Daten danach gelöscht.
- Beim Übergang zu Phase 4 bemerkte der Nutzer, dass im Home-Bereich (anders als in `screenshots/Home.jpg`) keine runden Übungsbilder erscheinen, und bat um Prüfung, ob das eine bewusste spätere Phase oder ein Versehen war. Untersuchung von `MVP.md`/`MVP_REVIEW.md`/`ROADMAP.md` ergab: **echtes Versehen**, keine dokumentierte Verschiebung (anders als z. B. die PR-Charts, die `MVP_REVIEW.md` explizit als "erst Phase 3" markiert). Auf Nutzerwunsch vor Phase 4 nachgezogen: Home-Karten zeigen jetzt bis zu 3 Übungen mit rundem Bild + Satzanzahl (`getWorkoutHistory()` dafür auf vier flache Queries umgestellt statt eines `count`-Embeds), Workout-Detail- und Übungshistorie-Karten zeigen ebenfalls das runde Übungsbild, und die eckigen Thumbnails im Übungs-Picker wurden auf `rounded-full` angeglichen. Gegen die Live-DB verifiziert (Test-Workout mit 4 Übungen unterschiedlicher Satzanzahl, `order_index`-Reihenfolge bestätigt), Test-Daten danach gelöscht. Details in `TODO.md`, Abschnitt „Nachgezogene Lücke".
- Danach `TODO.md`/`PROJECT_STATUS.md` um eine erste **Phase-4-Bestandsaufnahme** ergänzt (kein Code, reine Doku): sieben Punkte laut `ROADMAP.md`, mit Schema-/Code-Bereitschaftsprüfung je Punkt und zwei als klärungsbedürftig markiert (Kalender, Workout-Erinnerungen). Details in `TODO.md`, Abschnitt „Phase 4 der Roadmap".
- Nutzer wählte Punkt 1 (Notizen) als Phase-4-Einstieg. Umgesetzt: alle drei bereits vorhandenen `notes`-Spalten (`routines`, `workouts`, `workout_exercises`) an UI angebunden — Routine-Notizen im `RoutineForm`, Workout-Notizen und Pro-Übung-Notizen im aktiven Workout, alle drei sichtbar in den jeweiligen Detail-Ansichten. Keine neue Migration. Gegen die Live-DB verifiziert: alle drei Notiz-Ebenen einzeln geschrieben und mit den tatsächlichen API-Select-Formen zurückgelesen, Test-Daten danach gelöscht.
- Danach `TODO.md`/`PROJECT_STATUS.md` um eine erste **Phase-4-Bestandsaufnahme** ergänzt (kein Code, reine Doku, wie zuvor bei Phase 3): sieben Punkte laut `ROADMAP.md`, mit konkreter Prüfung der jeweiligen Schema-/Code-Bereitschaft (z. B. `notes`-Spalten und `is_custom`/`created_by` existieren bereits seit `0001_init.sql`, werden aber nirgends benutzt — verifiziert per `grep`). Zwei Punkte explizit als klärungsbedürftig markiert statt einer Annahme: „Kalender" (Historie-Ansicht vs. Trainingsplanung — `ROADMAP.md` sagt nicht welches) und „Workout-Erinnerungen" (lokale vs. serverseitige Push-Architektur, plus derselbe On-Device-Verifikations-Vorbehalt wie beim ausstehenden Gesamttest). Bewusst noch kein einzelner Punkt begonnen, bis der Nutzer priorisiert hat.
...
```
(neu) Build routine/workout/exercise notes (Phase 4, item 1)
...
- **Rekorde-Übersicht** (`records`): Liste aller Übungen mit `personal_records`, je Zeile Bestes Gewicht + Geschätztes 1RM, Tap → `exercise/[id]`. Erreichbar über einen Button im Profil-Tab.
...
- **Workout-Detail** (`workout/[id]`): Name/Datum/Dauer/Volumen/Anzahl Übungen im Header, darunter alle Übungen (mit rundem Bild) mit ihren Sätzen (Gewicht/Wiederholungen/completed-Status). Übungsname ist tappbar → `exercise/[id]`.
