...
Stand: 2026-07-24, Ende der Session. Phase 1 komplett, Design System komplett, **Phase 2 (Punkte 1–8) vollständig abgeschlossen** plus zwei ungeplante Zwischenschritte (Routinen-Liste, "Routine starten"). **Phase 3 (die komplette aktuelle Planungsgrundlage, Punkte 1–4) ebenfalls vollständig abgeschlossen.** Phase 4 ist fast abgeschlossen (Punkte 1–6 von 7 fertig: Notizen, Custom Exercises, Rest-Timer, Plattenrechner, Favoriten, Kalender — nur Workout-Erinnerungen fehlt noch). **Danach, außerhalb der Phase-4-Liste:** komplettes Redesign auf das Nocturne-Design-System (siehe „Nocturne-Redesign" unten). Der komplette Kernkreislauf **Registrieren → Routine erstellen → Routine starten → Workout durchführen → Workout beenden → Workout-Historie ansehen → Workout-Detail ansehen → Übungsdetail ansehen → Profil-Aggregate ansehen → Rekorde-Übersicht ansehen** ist Ende-zu-Ende gegen die echte Supabase-Datenbank verifiziert. Zwei reale, vorbestehende Bugs wurden dabei gefunden und gefixt (siehe unten). **Ausstehend ist nach wie vor nur der echte On-Device-Test** durch den Nutzer selbst — konnte in keiner Background-Job-Session durchgeführt werden.
...
- **Phase 4** (sieben Punkte laut `ROADMAP.md`, technische Bestandsaufnahme in `TODO.md`): Punkt 1 (Notizen) ist umgesetzt — alle drei bereits im Schema vorhandenen, aber ungenutzten `notes`-Spalten (Routine/Workout/Workout-Übung) sind jetzt an die UI angebunden. Punkt 2 (Custom Exercises) ist ebenfalls umgesetzt — neues Formular (`CustomExerciseForm.tsx`) + neuer Screen (`app/exercise/create.tsx`) + Einstiegspunkt im Picker, nutzt die seit `0001_init.sql` bestehenden, aber bis dahin ungenutzten `is_custom`/`created_by`-Spalten und RLS-Policies. Punkt 3 (Rest-Timer) ist ebenfalls umgesetzt — rein clientseitig, `routine_exercises.rest_seconds` wurde bereits geladen, aber bis dahin nie in den aktiven Workout-State übernommen; jetzt ein Countdown ab einem persistierten Endzeitpunkt (`restEndsAt`, App-Kill-sicher wie `startedAt`), getriggert beim Abhaken eines Satzes. Punkt 4 (Plattenrechner) ist ebenfalls umgesetzt — reine Client-Logik (Greedy-Algorithmus über ein Standard-Scheiben-Set), neuer Screen `plate-calculator`, Einstiegspunkt per neuem Icon pro Satzzeile im aktiven Workout. Punkt 5 (Favoriten) ist ebenfalls umgesetzt — neue, vierte Migration (`favorite_exercises`, Nutzer entschied sich für "nur Übungen"), Stern-Toggle im Picker und in der Übungsdetailseite, plus ein „Favoriten"-Filter-Chip im Picker. Punkt 6 (Kalender) ist ebenfalls umgesetzt — Nutzer entschied sich für die Historie-Ansicht (nicht Trainingsplanung); neuer Screen `app/calendar.tsx`, Monatsraster aus einer selbstgebauten Grid-Utility (keine neue Kalender-Lib), nutzt dieselbe `getWorkoutHistory()`-Query wie der Home-Tab, Tag antippen zeigt die Workouts dieses Tages über die wiederverwendete `WorkoutHistoryCard`. **Workout-Erinnerungen (Punkt 7) bleibt als einziger Punkt mit offener Architekturfrage (lokal vs. serverseitig) übrig** — damit ist Phase 4 bis auf diesen letzten Punkt vollständig.

## Nocturne-Redesign (nach Phase-4-Punkt-6, außerhalb der Roadmap)

Auf Nutzerwunsch importiert aus einem Claude-Design-Projekt (`claude.ai/design`, per `DesignSync`-Tool) und vollständig umgesetzt: neue Farb-, Typografie-, Spacing- und Radius-Tokens, Buttons als Akzent-Outlines statt gefüllter Flächen, Inter-Schriftfamilie (`@expo-google-fonts/inter`), kompletter Icon-Wechsel von `lucide-react-native` zu `phosphor-react-native` (10 betroffene Dateien, Dependency entfernt). Ausführliche Begründung, alle Interpretations-Entscheidungen (u. a. warum `light`/`dark`-Tokens trotz Einzel-Theme bestehen blieben, warum der Token-Name `primary` nicht zu `accent` umbenannt wurde, die Anpassung der Web-Spacing-Skala an Mobile-Werte) und bekannte Vereinfachungen stehen in `ARCHITECTURE.md`, Abschnitt „Design System". Wie der Rest der App nur gegen `tsc`/`lint`/`expo export` geprüft, nicht auf einem echten Gerät — visuelle Details (Fokus-Ringe, Press-Feedback, exakte Abstände) sind ungetestet.
...
- **Plattenrechner**: eigener Screen, berechnet die Plattenkombination pro Seite für ein Zielgewicht + Stangentyp (Standard/EZ/ohne Stange); Einstiegspunkt per Dumbbell-Icon direkt an jeder Satzzeile im aktiven Workout, vorausgefüllt mit dem eingetragenen Gewicht.
- **Favoriten**: Übungen favorisieren per Stern-Icon im Picker und in der Übungsdetailseite (`favorite_exercises`-Tabelle, RLS owner-only); „★ Favoriten"-Filter-Chip im Picker blendet die Ergebnisliste auf Favoriten ein.
- **Kalender** (`calendar`): Monatsraster der Trainings-Historie, Einstiegspunkt per Icon im Home-Tab-Header; Tag antippen zeigt die Workouts dieses Tages darunter, Tap auf ein Workout → `workout/[id]`.
- **DB-Schema**: `0001_init.sql` (vollständiges Schema aus `DATABASE.md`) + `0002_fix_finish_workout_personal_records.sql` (Bugfix) + `0003_add_estimated_1rm_personal_record.sql` (One Rep Max) + `0004_add_favorite_exercises.sql` (Favoriten).
