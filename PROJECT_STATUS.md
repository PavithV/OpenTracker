...
**Stand: 2026-07-26 (aktuellste Session).** Seit dem unten dokumentierten Stand (2026-07-24) zusätzlich passiert, nicht mehr in den folgenden Abschnitten nacherzählt: das **iOS-Redesign** hat Nocturne abgelöst (siehe `ARCHITECTURE.md`, Abschnitt „Design System" → „iOS-Redesign" für Details), ein vollständiges Produkt-/UX-/Architektur-/Sicherheitsaudit (`PRODUCT_AUDIT.md`) plus darauf aufbauender `LAUNCH_PLAN.md` (lokal beim Nutzer) wurden erstellt, und **Launch-Blocker #1 (Konto-Löschung, In-App-Teil)**, **#2 (App-Icon + Splash)**, **#3 (Error-/Loading-States)**, **#5 (fehlende Indizes + Wertebereichs-Constraints)**, **#6 (Accessibility-Labels für Icon-only-Buttons)**, **#7 (Toast + Undo + Abmelde-Bestätigung)** sowie **#8 (Vorheriger-Satz-Referenz beim Logging)** wurden umgesetzt — neue Migration `0006_add_delete_account.sql`, `deleteAccount()`-Client-API, „Konto löschen"-UI im Profil-Tab; echtes Icon/Splash-Branding (selbst generiert, kein KI-Bilddienst konfiguriert) plus erster echter nativer Splash Screen (`expo-splash-screen`, Hell/Dunkel-Paar) für dieses Projekt; konsistente Lade-/Fehlerzustände (neues `EmptyState`-`action`-Prop + `getErrorMessage()`-Helper) auf Home/Training/Profil/Records/Kalender/Picker; neue Migration `0007_add_workout_exercises_indexes_and_value_constraints.sql`; `accessibilityLabel`/`accessibilityRole` app-weit auf allen 18 Icon-only-Buttons plus den vier gemeinsamen Shared-Components (`Button`/`Card`/`ListItem`/`FilterChip`); neue Toast-Grundlage (`src/store/toast.store.ts` + `src/shared/components/Toast.tsx`) mit Undo für Routine-/Workout-Löschen sowie Abmelde-Bestätigung. **Zusätzlich, auf Nutzerwunsch**: Drag-and-Drop im aktiven Workout durch dieselben Hoch/Runter-Buttons wie im Routinen-Editor ersetzt, `react-native-draggable-flatlist` komplett aus dem Projekt entfernt. **#4 (Leaked-Password-Protection) bewusst zurückgestellt** — erfordert laut Supabase-Doku den Pro-Plan, Projekt läuft aktuell auf Free-Tier. Details in `TODO.md`, Abschnitte „Produkt-Audit + Launch-Plan", „Konto-Löschung", „App-Icon + Splash", „Error-/Loading-States", „Leaked-Password-Protection", „Fehlende Indizes + Wertebereichs-Constraints", „Accessibility-Labels für Icon-only-Buttons", „Toast + Undo + Abmelde-Bestätigung", „Drag-and-Drop im aktiven Workout durch Hoch/Runter-Buttons ersetzt" und „Vorheriger-Satz-Referenz beim Logging".
