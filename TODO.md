...
5. **Smoke-Test** — teilweise erledigt: Ein echter Simulator/Gerät-Test war in dieser (Background-Job-)Session nicht möglich (kein Android-SDK/Emulator, kein macOS/Xcode für iOS verfügbar). Stattdessen als Ersatz auf API-Ebene verifiziert — genau der Pfad, den `exercise/picker.tsx` nutzt: `GET /rest/v1/exercises` mit dem `anon`-Key (RLS-Policy "exercises are readable by everyone") liefert echte Zeilen zurück, und ein Bild-URL aus dem `exercise-media`-Bucket ist öffentlich per HTTP 200 erreichbar. **Noch offen**: der eigentliche On-Device-Test (`npm run ios`/`npm run android`, Login/Registrierung, Navigation zu `exercise/picker`) — das muss der Nutzer selbst auf einer Maschine mit Simulator/Emulator/Gerät durchführen.
