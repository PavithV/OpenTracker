...
**Die komplette geplante Phase-2-Liste (Punkte 1–8) ist abgeschlossen.** Nächster sinnvoller Schritt ist entweder (a) der ausstehende echte On-Device-Test durch den Nutzer selbst (siehe „Offene Aufgaben" oben in `PROJECT_STATUS.md`), oder (b) der Einstieg in Phase 3 laut `ROADMAP.md` (Diagramme, One Rep Max, Muskel-Split, `personal_records` aktiv anzeigen) — das PR-Chart, das in Punkt 7 bewusst weggelassen wurde, gehört genau hierher.
...
- `personal_records` wird seit Punkt 5 serverseitig befüllt und seit Punkt 7 im Übungsdetail als einfache Karte gelesen, aber noch nirgends visualisiert (Diagramm) oder aggregiert dargestellt — das ist explizit Phase 3 laut `ROADMAP.md`.
